...
The graphical model for the PoE given in Q\ref{q:poe-hierarchical-models} can be drawn for vector variables $\vy$, $\vf$, $\vtheta$ as in Fig.~\ref{fig:gm-hierarchical} (left-hand side). However, we can be more specific about the relationship between variables by splitting up the vectors $\vf$ and $\vy$ into scalar variable nodes (Fig.~\ref{fig:gm-hie¯rarchical}, centre and right-hand side). As we have not been told which variables were observed, all nodes are left unshaded.

\begin{figure}[h]
\centering
\begin{tikzpicture}
  \node[latent](theta) at (0,2.8) {$\vtheta$};
  \node[latent](f) at (0,1.4) {$\vf$};
  \node[latent](y) at (0,0) {$\vy$};
  \edge{theta}{f};
  \edge{f}{y};
...
\hspace{1.0cm}
\begin{tikzpicture}
  \node[latent](theta) at (0.3,2.8) {$\vtheta$};
  \node[latent](f1) at (-1,1.4) {$f_1$};
  \node[latent](y1) at (-1,0) {$y_1$};
  \node[latent](f2) at (0,1.4) {$f_2$};
  \node[latent](y2) at (0,0) {$y_2$};
  \node[latent](fN) at (1.4,1.4) {$f_N$};
  \node[latent](yN) at (1.4,0) {$y_N$};
  \edge{theta}{f1};
  \edge{theta}{f2};
  \edge{theta}{fN};
  \edge{f1}{y1};
  \edge{f2}{y2};
  \edge{fN}{yN};
  \node(fdots) at (0.7,1.4) {$\ldots$};
  \node(ydots) at (0.7,0) {$\ldots$};
\end{tikzpicture}
\hspace{1.0cm}
\begin{tikzpicture}
  \node[latent](theta) at (0,2.8) {$\vtheta$};
  \node[latent](fn) at (0,1.4) {$f_n$};
  \node[latent](yn) at (0,0) {$y_n$};
  \edge{theta}{fn};
  \edge{fn}{yn};
  \plate[xshift=0.3cm] {fy} {(fn)(yn)} {\hspace{0.3cm}$n=1:N$};
\end{tikzpicture}
